...
public class ClienteDAOImpl implements ClienteDAO {
    ...
    public void delete(Connection conn, Integer idCliente) throws Exception {
        ...
        while(myRs.next()) {
        	...
        	myStmt.setInt(1, myRs.getInt("id_aluguel"));

        	myStmt.execute();
        }
        
        myStmt = conn.prepareStatement("delete from en_aluguel where id_cliente = ?");
        
        myStmt.setInt(1, idCliente);
        
        myStmt.execute();
        
        myStmt = conn.prepareStatement("delete from en_cliente where id_cliente = ?");
